...
function updateImportsInFile(filePath) {
  try {
    ...
    for (const [component, newPath] of Object.entries(componentMapping)) {
      ...
      // Handle relative imports (for components that import other components)
      const relativeImportRegex = new RegExp(`from ['"]\.?/${component}['"]`, 'g')
      const relativeNewImport = `from '@/components/${newPath}'`

      if (relativeImportRegex.test(content)) {
        content = content.replace(relativeImportRegex, relativeNewImport)
        updated = true
      }
    }
