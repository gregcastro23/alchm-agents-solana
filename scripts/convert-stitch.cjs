...
const CONFLICTING_TOKENS =
  /(^|[\s"':])((?:bg|text|border|ring|ring-offset|shadow|from|via|to|divide|placeholder|decoration|outline|accent|caret|fill|stroke)(?:-[xylrtbse])?-)(primary|secondary|background|surface)(?![\w-])/g

const rewriteConflictingTokens = content => content.replace(CONFLICTING_TOKENS, '$1$2st-$3')

const dirs = fs
  .readdirSync(STITCH_DIR)
  .filter(d => fs.statSync(path.join(STITCH_DIR, d)).isDirectory())
