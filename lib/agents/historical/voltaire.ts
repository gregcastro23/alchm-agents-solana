...
export const VOLTAIRE: HistoricalCraftedAgent = {
  ...
  consciousness: {
    natalChart: {
      provenance: 'authored',
      provenanceNote:
        "AUTHORED, and demonstrably not a measurement: Mercury is 34.0 degrees from the Sun (an inferior planet, max ~28). These positions were entered by hand, not derived from an ephemeris. WHY THIS WAS NOT REPLACED WITH A COMPUTED CHART (checked 2026-07-28, by scripts/compute-historical-natal-charts.py, which refuses to emit a chart for this subject): Refused on TWO independent grounds, the second of which this script measured rather than assumed. (1) The date is contested: en.wikipedia.org/wiki/Voltaire gives 21 November 1694 from the parish register (he was baptised on 22 November), but notes 'Some speculation surrounds Voltaire's date of birth, because he claimed he was born on 20 February 1694 as the illegitimate son of a nobleman' - the two candidates are nine months apart. (2) Even granting the November date, no birth time is recorded, and at 12:00 local mean time in Paris the Sun is at Scorpio 29.74 deg - it crosses into Sagittarius about six hours later. So the birth date alone does not determine even his SUN SIGN, the most load-bearing number in the chart. Computing it would mean picking one of two signs by coin flip and labelling the result 'computed'. A recorded birth time would fix ground (2); only new documentary evidence fixes ground (1). A computed chart becomes possible only if a documented birth date turns up; a birth TIME would additionally be needed before any ascendant or house placement could be claimed.",
      planets: {
