...
function fallbackBatchResponse(body: unknown) {
  const agents =
    body && typeof body === 'object' && Array.isArray((body as { agents?: unknown }).agents)
      ? ((body as { agents: BirthChartData[] }).agents as BirthChartData[])
      : []
    ...
    data: {
      ...
      results: Object.fromEntries(
        agents.map(agent => [
          ...
          generateFallbackConsciousnessData(agent),
        ])
      ),
    },
  })
...
async function proxyConsciousnessRequest(path: string, body: unknown) {
  try {
    ...
    if (response.status === 404) {
      return fallbackBatchResponse(body)
    }
